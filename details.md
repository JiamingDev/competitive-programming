...
7.stoi 是 C++11 标准引入的一个标准库函数，全称是 String TO Intger,定义在 <string> 头文件中。它的功能是将字符串 std::string 转换为整数 int,前导 0 会被自动忽略
```cpp
string s = "123";
...
```
202.tuple也可以放二元。
203.给要置于set中的结构体重载<，必须要有2个const，因为加入set中的元素都变为了const，如果你的函数参数不是const,则不能用于const变量的比较，因此会报错。sort中的也一样，也要加const.
204.非 const 传给 const 形参：允许（向上兼容,原先有读写权，现在告诉函数使用只读处理，权限缩小，可以）const 传给非 const 形参：禁止（权限越级，原先只读，现在要求它读写，权限放大，不允许）
205.using 只能代表“类型”，不能代表“代码逻辑”,即不能和#define一样给for循环起别名，因为现在一般用不到起别名了（有auto）
206.不要用define定义类型别名，因为它只是文本替换，而不是类型定义，using和typedef才是类型定义
```cpp
#define PINT int*
PINT a, b;//实际结果：a 是一个 int*（指针），而 b 只是一个普通的 int！原因：* 在 C++ 语法中紧跟变量名
```
207.不能直接改变set中的元素值，因为他们都被设为了const,即不能：set<int>s;……for(auto&x:s)x=1;但可以for(auto x:s)x=1;因为这不是直接更改set的元素。
208.memset的时间复杂度是O(n),但它的常数很小，远比手写的for循环快。
209.pair<int,int>a[maxn];memset(a,0,sizeof(a));这样会出现警告，因为pair被视作非平凡类型（即使里面只有int),不能直接赋值0，
所以以后直接用struct替代pair就行，这样在struct中只有简单类型时可以直接赋值0.
210.for循环进行完一次再进行下一次时先执行i++；再进行条件判断。
211.unordered_set必须有==和哈希函数,set必须有<
212.abs()可以求整数或浮点数的绝对值，它是重载的。
213.goto语句的标记的作用域是函数内部，所以可以在不同函数中定义相同名字的标记，它们不会冲突。
214.任何在一对花括号 { } 内定义的变量，其生命周期仅限于这对花括号。
```cpp
if (condition) {
    int temp = 10;
    // temp 在这里可用
...
```
215.// 行号 95
for (int j = 1; j <= (i == d + 1) ? i : i - 1; j++)
在 C++ 中，<= 的优先级 高于 ? :。编译器看待这句代码的眼光是这样的：
for (int j = 1; (j <= (i == d + 1)) ? i : (i - 1); j++)
216.puts 的核心特性就是：自动在输出字符串后追加一个换行符 \n
217.<<（移位运算符）的优先级比 &（位与运算符）高。所以可以写if(s&1<<n)
218.long long的数量级是10^18
219.std::bitset 就像是一个可以无限长度（在编译期确定）且自带位运算功能的 bool 数组。
bitset 的长度必须是编译期常量（即 const 或字面量）
```cpp
#include <bitset>
std::bitset<20> bs;           // 创建一个 20 位的 bitset，初始全为 0
std::bitset<20> bs2(0xfa);    // 用十六进制/十进制初始化（二进制：00000000000011111010）
std::bitset<20> bs3("1101");  // 用字符串初始化（自动高位补 0）
```
它重载了所有的位运算符：&, |, ^, ~, <<, >>
| 函数 | 功能 | 竞赛用途 |
| --- | --- | --- |
| bs.set(i) | 将第 i 位设为 1 | 标记某个状态存在 |
| bs.reset(i) | 将第 i 位设为 0 | 移除标记 |
| bs.test(i) | 返回第 i 位是 0 还是 1 | 安全查询（比 bs[i] 更稳） |
| bs.count() | 返回 1 的个数 | 计算集合大小（相当于 __builtin_popcount） |
| bs.any() | 是否存在 1 | 快速判断集合非空 |
| bs.none() | 是否全为 0 | 快速判断集合为空 |
| bs.all() | 是否全为 1 | 检查是否填满 |
| bs.to_ulong() | 转为 unsigned long | 输出或与其他整数运算 |
| bs.to_string() | 转为二进制字符串 | 方便调试打印 |
可以这样输出二进制形式：cout<<bitset<20>(x);//它重载了<<运算符
220.左移溢出：最高位丢弃，低位补 0(前提:位移量小于类型位宽)如果一次位移>=类型位宽就是未定义行为了，系统可能给你自动取余了，如d>=32时可能处理为：a << (d % 32)
1ULL << 63 之后再 << 1 确定等于 0，和1ULL<<64的结果可能截然不同。但最好还是用x&((1<<n)-1)的方法掩码处理，因为不确定有没有溢出风险。
221.for循环中的i变成绿色时可能是内部还有个for循环用的是i
222.long long有1 位符号位，63 位数值位
223.将二进制的某一位设为0：
```cpp
x = x & ~(1LL << i);
// 简写形式
x &= ~(1LL << i);
```
224.fgets可以直接作为判断条件，指针可以隐式转换为布尔值：非空指针为 true，NULL 指针为 false
225.如果一个变量处于作用域内，它的初始化必须被执行。goto 跳过了初始化，但 c2 的生命周期还在继续（直到第 63 行的大括号结束），这会导致程序状态不确定，因此编译器直接报错。
226.异或运算^的逻辑是：相同为 0，不同为 1
227.cin.ignore(1024, '\n'); // 意思是：丢弃最多 1024 个字符，或者直到遇到 '\n'.它会把 \n 从缓冲区中拿走并丢弃。
更推荐的方法:
```cpp
//下面两者都能实现消除所有空白字符但不会读入非空白字符。
cin >> ws; // ws 是一个流操纵符，跳过前导空白
scanf(" "); // 关键点：这个空格占位符会吃掉所有连续的空白字符，直到遇到第一个非空白字符
```
228.现在的换行符主要有两种情况（其它(classic mac:\r)都是很老旧的版本了，不用考虑）1.windows:\r\n。2.unix/Linux:\n.
229.将单个字符转为string并拼接到另一个string的方法：string(size_t n, char c)，表示创建一个包含 n 个字符 c 的字符串
```cpp
char c = 'a';
string s(1, c); // 结果为 "a"
//方法二：
...
s += c; // 自动完成类型转换并追加
```
230.默认情况下，stringstream 会忽略前导零，将其作为普通的十进制数字读入。
如果输入字符串是 "0100"，执行 ss >> d 后，d 的值将是 100
如果输入字符串是 "0"，d 的值将是 0
static 局部变量的初始化语句（即 = 及其后面的部分）只在程序第一次执行到这一行时运行。后续再调用该函数时，这一行会被直接跳过。
静态局部变量 (static int x)的作用域也是仅限该函数内（外部不可见）
想让函数中的一个局部变量初始化为0且后面再次调用时不用再初始化为0，就用static定义，这通常搭配时间戳，设置一个数，每次进入时都加一，如static int t;(static初始化语句，只进行一次)t++;(普通语句，每次进入时都会执行)这样后面判断是否在此次调用中赋了值时，就判断是否等于这个时间戳就行，避免了每次调用时的重新开销。
231.# 编译：-O2 是开启优化（比赛必加），-Wall 是显示所有警告,-o是取exe文件的名字(没有的话默认是a.exe)
g++ -O2 -std=c++17 -Wall practice1.cpp -o practice1
# 运行
./practice1.exe  (Windows)
./practice1      (Linux)
编译并执行：g++ -O2 -Wall -std=c++17 uva11846.cpp -o uva11846 && .\uva11846.exe
当你执行 g++ uva101.cpp -o uva101 时，如果编译成功，编译器会自动杀掉旧的 uva101.exe 并替换成全新的。这能保证你的文件夹始终干净整洁。
❌危险动作（分两次敲）：
g++ uva101.cpp -o uva101 （假设这步因为语法错误失败了，没有生成新文件）
./uva101.exe （你手动敲了运行，结果系统跑了五分钟前生成的旧版程序）
✅ 专业动作（用 && 连起来）：
g++ uva101.cpp -o uva101 && ./uva101.exe
它的逻辑是： 只有左边成功了，才会执行右边。
如果你代码写错了，编译报错，右边的 && ./uva101.exe 根本不会被触发。
屏幕上只会显示红色的报错信息，而不会跳出旧的运行结果。
232.long long 的数值范围是-2^63~2^63-1,+(-)9*10^18
unsigned long long:0~2^64-1,1.84*10^19
__int128:-2^127~2^127-1,1.7*10^38:使用场景：计算 a*b%m。即使 $a, b, m$ 都在 long long 范围内，a*b 也会溢出。直接(__int128)a * b % m,也可用于较大的二进制储存状态，__int128 x = (__int128)1 << 100;无法直接通过 cin / cout 或 scanf / printf 读写。你需要手写快读（Quick Read）和快写。
即读入一个字符就转为数字加到__int128变量中再进一位，这样循环读入，输出的时候也只能一部分一部分地输出出来。
233.不能让字符数组的定义长度恰好等于输入字符的长度，因为这样可能会导致自动加的'\0'越界访问了。
234.两个反斜杠表示一个反斜杠字符：'\\' / "\\";
235.max({a, b, c}) （C++11 初始化列表写法，更推荐，简洁且易于扩展）
236.a.data() 返回一个指向 vector 内部存储数组的第一个元素的指针（即该类型的原始指针）
```cpp
int maxsum_linear(int *A, int n){}
vector<int> a(n);
maxsum_linear(a.data(), n);
```
237.remove_if：将不符合条件（即需要删除）的元素移动到容器末尾，并返回一个指向“逻辑末尾”的迭代器。
```cpp
// 假设我们要保留所有右端点 r > 10 的区间
v.erase(remove_if(v.begin(), v.end(), [](const interval &inv) {
    return inv.r <= 10; // 注意：这里写的是“删除条件”，即不符合条件的
}), v.end());
```
c++20:
```cpp
#include <vector>
std::erase_if(v, [](const interval &inv) {
    return inv.r <= 10; 
});
```
238.if(!f++)的判断顺序也是先取值判断再加1。!的优先级小于后置自增，即相当于!(f++)而非(!f)++;
239.copy(a, a + n, a2);这样可以只拷贝有效元素。
240.cout<<'A'+1;这个的输出结果是66,char会被提升为int.
241.结构体内定义 < 运算符：这被称为默认排序准则。如果你调用 sort(a, a + n) 而不传第三个参数，编译器会自动寻找该重载。
自定义 cmp 函数/仿函数：这被称为显式指定准则。如果你调用 sort(a, a + n, cmp)，编译器会优先使用你传入的 cmp，而忽略结构体内部定义的 <。
242.double atan2(double y, double x);
(x,y)在第一二象限返回0~pi,三四象限返回-pi~0;
243.set的迭代器it只能自增或自减，不能写it+1,因为没有重载+运算。
244.s.erase(it);it++;是不行的，it已经被销毁了，s.erase(it++);是可以的，它的执行原理如下：
...
251.遇到复杂构造最优解题，如求某种数列求值公式可能的最大值，如果发现要讨论复杂数学分类和公式，赶紧想到可以找到最优解条件，然后把数组排列成那个条件，直接模拟求值。